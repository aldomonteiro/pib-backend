...
var _usersController = require("../controllers/usersController");function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
router.get("/:id", function (req, res) {
  _users.default.findOne({ id: req.params.id }).then(function (result) {
    res.json(result);
  });
});
...
router.put('/:id', function (req, res) {
  ...
  _users.default.findOneAndUpdate({ id: req.params.id }, updatedElement).
  then(function (oldResult) {
    _users.default.findOne({ id: req.params.id }).
    then(function (newResult) {
      ...
      console.log(err);
      res.status(500).json({ success: false, msg: "Something went wrong. ".concat(err) });
      return;
    });
  }).
  catch(function (err) {
...
router.delete('/:id', function (req, res) {
  _users.default.findOneAndRemove({ id: req.params.id }).
